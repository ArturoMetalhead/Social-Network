...
def getShaRepr(data: str):
    """
    Get the SHA1 hash of the data and return it as an integer.
    """
...
class ChordNodeReference:
    ...
    def _send_data(self, op: int, data: str = None) -> bytes:
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                print(f"sending data from {self.ip},{self.port} to ")
    ...
    def succ(self) -> 'ChordNodeReference':
        ...
        if not response or len(response) < 2:
            print(f"Received invalid response for GET_SUCCESSOR: {response}")
            return None
        return ChordNodeReference(int(response[0]), response[1], self.port)
...
class ChordNode:
    def __init__(self, id: int, ip: str, port: int = 8001, m: int = 160):
        ...
        # # Populate finger table initially
    ...
    def join(self, node: 'ChordNodeReference'):
        ...
        print(f"\nIniciando join al nodo {self.id}")
        if node:
            self.pred = None
            ...
            self.succ = self.ref
            self.pred = None
        print(f"\nFinalizando join al nodo {self.id}")
    ...
    def fix_fingers(self):
        """
            Regularly fix fingers.
        """
        #while True:
        try:
            print("Fix fingers entry")
            i = self.next
            start = (self.id + 2 ** i) % (2 ** self.m)
            self.finger[i] = self.find_succ(start)
